...
from automation_status import init_automation_tables, latest_automation_run, automation_summary
from debug_whoop_dates import latest_whoop_date_diagnostic

validate_config()
app = FastAPI(title="WHOOP Health Intelligence", version="0.4.4-debug")
app.add_middleware(SessionMiddleware, secret_key=SESSION_SECRET, same_site="lax", https_only=True)

@app.on_event("startup")
def startup():
    init_db(); init_analytics(); init_baselines(); init_automation_tables()
...
async def home(request: Request):
    ...
    return """<html><body style="font-family:Arial;max-width:980px;margin:50px auto"><h1>WHOOP Health Intelligence</h1><p><b>Phase 4C.2 Diagnostic: WHOOP Date Alignment</b></p><ul>
    <li><a href="/debug/latest-whoop-records">Inspect latest WHOOP timestamps</a></li>
...
@app.get("/health")
async def health():
    return {"status":"ok","phase":"4C.2-diagnostic","version":"0.4.4-debug"}
...
async def debug_latest_whoop_records(request: Request):
