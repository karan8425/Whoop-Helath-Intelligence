...
app = FastAPI(title="WHOOP Health Intelligence", version="0.3.2")
app.add_middleware(
    SessionMiddleware,
...
async def home(request: Request):
    ...
    return """<html><body style="font-family:Arial;max-width:920px;margin:50px auto">
    <h1>WHOOP Health Intelligence</h1>
    <p><b>Phase 3B: Personal Baseline Engine</b></p>
    <p>Each day is compared with the preceding 7, 14, 30 and 90 calendar days. Missing physiological values are excluded, never treated as zero.</p>
    <ul>
      <li><a href="/analytics/baselines/rebuild">Build / rebuild personal baselines</a></li>
      <li><a href="/analytics/baselines/validate">Validate baseline engine</a></li>
      <li><a href="/analytics/baselines/latest">View latest personal baselines</a></li>
      <li><a href="/analytics/baselines/history?metric=hrv_rmssd_milli&limit=30">View 30-day HRV baseline history</a></li>
...
@app.get("/health")
async def health():
    return {"status": "ok", "phase": "3B", "version": "0.3.2"}
...
async def rebuild(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Baseline rebuild failed: {exc}"
        ) from exc

@app.get("/analytics/baselines/latest")
async def latest(request: Request):
    require_admin(request)
    return {"status": "ok", **latest_baselines()}
...
async def history(request: Request, metric: str = "hrv_rmssd_milli", limit: int = 30):
    require_admin(request)
    try:
        return {"status": "ok", **metric_history(metric, limit)}
...
async def metrics(request: Request):
    require_admin(request)
    return {"status": "ok", "metrics": sorted(METRICS.keys())}
